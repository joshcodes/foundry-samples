...
|---|---|---|
| **AI** | `ServiceIdentity` service principal — your hired instance's identity. This is the principal that runs the OBO token exchange at every turn. | `894f4f2b-…` "Release Captain" |
| **AU** | `agentUser`-type Entra user — the user identity the AI acts on behalf of. | `734a97bc-…` `releasecaptain@yourtenant.onmicrosoft.com` |
...
display name of the hired worker (e.g. "Release Captain"), or with
Microsoft Graph:
```powershell
...
- **Multiple AI agents in one chat racing**: when you have several AI agents
  added to the same Teams chat (e.g. Release Captain + another teammate), the SDK can drop one
