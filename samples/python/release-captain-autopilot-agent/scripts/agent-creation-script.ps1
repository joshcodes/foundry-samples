  ...
  $ModelDeployment = if ($env:MODEL_NAME) { $env:MODEL_NAME } else { "gpt-5.3-chat" }
  ...
  $agentCreationBody = @{
      definition = @{
          kind = "hosted"
          image = "$($AzureContainerRegistryEndpoint)/release-captain-agent:latest"
          cpu = "2"
          memory = "4Gi"
          environment_variables = @{
              # Runtime override of the model deployment baked into the image.
              # The Python agent (`agent.py`) reads `ModelDeployment` to pick
              # which Azure OpenAI deployment to call. Setting it here lets us
              # swap models without rebuilding the container image.
      ...
      description = "Release Captain — Foundry A365 autopilot agent that runs the release-coordination chase loop."
      agent_endpoint = @{
        protocols = @("activity")
      }
      blueprint_reference = @{
