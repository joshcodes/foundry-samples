...
$aiObjectId = az ad sp show --id $AiClientId --query id -o tsv 2>$null
if (-not $aiObjectId) {
...
$mcpSpId = (Invoke-RestMethod -Uri "https://graph.microsoft.com/v1.0/servicePrincipals(appId='$mcpAppId')?`$select=id" -Headers @{Authorization="Bearer $graphToken"}).id
$apxSpId = (Invoke-RestMethod -Uri "https://graph.microsoft.com/v1.0/servicePrincipals(appId='$apxAppId')?`$select=id" -Headers @{Authorization="Bearer $graphToken"}).id
...
function Set-OAuth2Grant {
    ...
    $existing = Invoke-RestMethod -Uri "https://graph.microsoft.com/v1.0/oauth2PermissionGrants?`$filter=clientId eq '$Client' and resourceId eq '$Resource'" -Headers $gh
    if ($existing.value -and $existing.value.Count -gt 0) {
        ...
        if (-not $missing) {
            ...
            Invoke-RestMethod -Method PATCH -Uri "https://graph.microsoft.com/v1.0/oauth2PermissionGrants/$($g.id)" -Headers $gh -Body $body | Out-Null
        ...
        Invoke-RestMethod -Method POST -Uri "https://graph.microsoft.com/v1.0/oauth2PermissionGrants" -Headers $gh -Body $body | Out-Null
